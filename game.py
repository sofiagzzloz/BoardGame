...
def draw_grid():
    for c in range(COLUMN_COUNT):
        for r in range(ROW_COUNT):
            # Draw the blue background squares
            ...
            if col >= 0 and col < COLUMN_COUNT and is_valid_location(col):
                ...
                if row is not None:
                    ...
                    if check_win(current_player):
                        label = font.render(f"Player {current_player} wins!", 1, RED if current_player == 1 else YELLOW)
                    ...
                    # Switch players
                    current_player = 3 - current_player  # This switches between 1 and 2
